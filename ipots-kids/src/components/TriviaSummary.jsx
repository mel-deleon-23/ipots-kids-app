...
import '../styles/TriviaSummary.css';

const TriviaSummary = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { gemsEarned, totalQuestions, level } = location.state || { gemsEarned: 0, totalQuestions: 0, level: null };
  ...
  return (
    <div className="trivia-summary">
      ...
      <div className="actions">
        <button className="action-button btn btn-primary mt-3">Next Game</button>
        <button className="action-button btn btn-info mt-3" onClick={handlePlayAgain}>Play Again</button>
        <Link className="home-button btn btn-secondary mt-3" to={`/trivia`}>Home</Link>
      </div>
    </div>
  );
};
